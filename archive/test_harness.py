...
def run_test_harness():
    """
    Run team_cli_dlc.py and surface its output directly.
    By default, sends a single "finalize" command so you can see the autofill output.
    """
    parser = argparse.ArgumentParser(description="Simple harness to exercise team_cli_dlc.py and view output.")
    parser.add_argument("--python", default=sys.executable, help="Python interpreter to use (defaults to current).")
    parser.add_argument("--script", default="team_cli_dlc.py", help="Target CLI script.")
    parser.add_argument(
        "--commands",
        default="finalize",
