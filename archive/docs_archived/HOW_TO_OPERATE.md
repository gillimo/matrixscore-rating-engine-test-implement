## How To Operate (DLC) - Gemini Agent (Updated 2025-12-17)
...
*   **Permissions & Logging Footer:** Operations are confined to `OneDrive/Desktop/teambuilder dlc`. All generated log files must include the current permissions footer from `PERMISSIONS.md`:  
    `Permissions: full access to OneDrive/Desktop/teambuilder dlc granted by user on 2025-12-17.`  
...
The CLI operates interactively, allowing users to add Pok̩mon, manage the team, and finalize the process.

*   **Adding Pok̩mon:** Type a Pok̩mon's name (and optional level cap) at the prompt:
...
*   **Team Management Commands:**
    ...
    *   `drop <pokemon_name>`: Removes the specified Pok̩mon from the current team.
...
To run the tool in a non-interactive, test-like mode, execute the `team_cli_dlc.py` script directly with commands as arguments or piped input.

*   **Official Autofill Test (empty team):**
    `echo "finalize" | python team_cli_dlc.py`
