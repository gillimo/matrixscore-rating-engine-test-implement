## Ticket: Official PokÃ©mon Look & Feel for Tk UI

Owner: Codex-Autonomy  
Status: Resolved (2025-12-17)

### Objectives
- Align the Tk UI with a clean â€œofficialâ€ PokÃ©mon presentation while keeping the current lazy-loading/lite behavior.
...
### Requirements
- **Palette**: Use official type colors for cards/badges; dual-types get a true gradient (top->bottom or diagonal) blending the two type colors. Global UI uses PokÃ©dex primaries (red `#EE1515`, blue `#3B4CCA`, yellow `#FFCB05`, dark `#1D1E2C`, light `#FFF7D6`).
...
- **Metrics bar**: Solid PokÃ©dex-blue strip with white text; shows payload scores exactly; â€œTell me moreâ€ as a red button.
...
- **â€œI donâ€™t have thisâ€**: One-shot clear + auto-fill from upgrades, then re-score locally; banner updates without rerunning CLI.
...
4) Ensure metrics bar uses payload scores on initial load; local re-score only on â€œI donâ€™t have thisâ€.
...
### Outcome
- Palette themed with PokÃ©dex primaries; type tints applied with dual-type blends; high-contrast text; buttons/status/metrics bars styled.
