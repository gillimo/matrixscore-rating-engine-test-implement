...
-   **File Versioning:** The current main development script is `team_cli_v5.py`. The previous `team_cli_v3.py` has been archived to `archive/team_cli_v3.py.archive`.
...
-   **Filesystem:** workspace-write within OneDrive Desktop; `PERMISSIONS.md` is the authoritative inherited-permissions-source; include footer (`Permissions: full access to OneDrive/Desktop/teambuilder_v5 granted by user on 2025-12-17.`) on logs.
-   **Network:** restricted; PokeAPI typically reachable (required at runtime).
...
-   **Access practice:** only issue visible PowerShell commands against `OneDrive/Desktop/teambuilder_v5`; no hidden processes or destructive commands; network used only when running the demo (PokeAPI).
